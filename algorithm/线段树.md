...
- 求逆序数
    - 普通线段树的做法：
        ...
        - 按顺序插入序列，对新插入项，其在区间`[1,n]`中排k，则求区间`[k+1, n]`中插入值的个数cnt_k。（注意k+1是没有重复项的前提下）
...
- 其实权值线段树用动态开点线段树可以很自然的实现，不需要考虑离散化，空间大小也不会超。


https://zhuanlan.zhihu.com/p/492995124
```cpp
struct node {
    int sum;
    int l, r;
